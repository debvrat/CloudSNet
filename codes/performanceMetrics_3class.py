...
def metricCalc_3class(confMat):
	pa_c = confMat[0,0] / np.sum(confMat[:,0])
	ua_c = confMat[0,0] / np.sum(confMat[0,:]) 
	pa_sh = confMat[1,1] / np.sum(confMat[:,1])
	ua_sh = confMat[1,1] / np.sum(confMat[1,:])
	pa_r = confMat[2,2] / np.sum(confMat[:,2])
	ua_r = confMat[2,2] / np.sum(confMat[2,:])
	f_c = stats.hmean([pa_c, ua_c])
	#f_sn = stats.hmean([pa_sn, ua_sn])
	#f_2cls = np.mean([f_c, f_sn])
	oa = np.trace(confMat)/np.sum(confMat)
	return pa_c*100, ua_c*100, pa_sh*100, ua_sh*100, f_c*100, pa_r*100, ua_r*100, oa*100
...
for file in fileList:
	...
	totalVec = metricCalc_3class(total)
	testVec = metricCalc_3class(test)
	trainVec = metricCalc_3class(train)
